...
def fetch_employee_data():
    ...
    try:
        ...
        attrition_rate = (
            former_emp / total_population * 100 if total_population > 0 else 0.0
        )
        ...
        cur.execute("""
            SELECT
                ...
                SUM(CASE WHEN termination_type = 'Termination' THEN 1 ELSE 0 END),
                SUM(CASE WHEN termination_type = 'Resignation' THEN 1 ELSE 0 END),
                SUM(
                    CASE
                        WHEN LOWER(termination_sub_reason) LIKE '%family%'
                          OR LOWER(termination_sub_reason) LIKE '%marriage%'
                          OR LOWER(termination_sub_reason) LIKE '%child%'
                          OR LOWER(termination_sub_reason) LIKE '%parent%'
                        THEN 1 ELSE 0
                    END
                )
            FROM former_employees
        ...
        cur.execute("""
            SELECT ISNULL(
                SUM(
                    basic_salary
                    + ISNULL(housing_allowance, 0)
                    + ISNULL(transport_allowance, 0)
                ),
                0
            )
            FROM former_employees
...
def fetch_performance_risks():
    ...
    if perf_df is None or perf_df.empty:
        ...
        exceed_pct = (
            len(exceed_df) / total_reviews * 100 if total_reviews > 0 else 0.0
        )

        top_row = perf_df.sort_values(
